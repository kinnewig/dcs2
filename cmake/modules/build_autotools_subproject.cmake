function(build_autotools_subproject_with_custom_configure_and_update name configure update)
  ...
  if (DEFINED ${name_upper}_CUSTOM_TAG)
    ...
    message("Using custom git tag for ${name_upper}: ${${name_upper}_CUSTOM_URL}")
  endif()
  ...
  ExternalProject_Add_Step(
    ${name} ${name}_symlink
    COMMAND bash -c "[ -d \"${${name_upper}_DIR}/lib\" ] || ( [ -d \"${${name_upper}_DIR}/lib64\" ] && ln -sf \"${${name_upper}_DIR}/lib64\" \"${${name_upper}_DIR}/lib\" )"
  ...
  ExternalProject_Add_Step(
    ...
    COMMAND bash -c "[ -d \"${${name_upper}_DIR}/lib64\" ] || ( [ -d \"${${name_upper}_DIR}/lib\" ] && ln -sf \"${${name_upper}_DIR}/lib\" \"${${name_upper}_DIR}/lib64\" )"
    WORKING_DIRECTORY ${${name_upper}_DIR}
    DEPENDEES install
  )
endfunction()
...
function(build_autotools_subproject_with_custom_configure name configure)
  build_autotools_subproject_with_custom_configure_and_update(${name} ${configure} "")
endfunction()


function(build_autotools_subproject name)
  build_autotools_subproject_with_custom_configure_and_update(${name} "./configure" "")
endfunction()
