...
def update_package(database, package, dry_run=False):
    ...
    try:
        ...
        if current_version and parse_version(cmake_tag) <= parse_version(current_version):
            print(f"{package} is already up to date (current version: {cmake_tag}). Nothing to do.")
            ...
            if "git" not in info:
                for subpkg, subdatabase in database[package].items():
                    if isinstance(subdatabase, dict) and "git" in subdatabase:
                        ...
                        update_counter += update_package(database[package], subpkg.lower(), dry_run=args.dry_run)
                        package_counter += 1
                        print(f"\n")
            else:
                # update the package:
